...
public class DimatisItem extends Item implements AscunauticDimatis {
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        if (stack.isOf(ItemInit.ARBUNE_DIMATIS)) {
            tooltip.add(Text.translatable("text.boleatte.dimatis.veret_trusting").setStyle(Style.EMPTY.withColor(0xF9A357)));
        } else if (stack.isOf(ItemInit.BOTAKOA_DIMATIS)) {
            tooltip.add(Text.translatable("text.boleatte.dimatis.beusere_trusting").setStyle(Style.EMPTY.withColor(0x686DA5)));
        } else if (stack.isOf(ItemInit.RASIORE_DIMATIS)) {
            tooltip.add(Text.translatable("text.boleatte.dimatis.gela_trusting").setStyle(Style.EMPTY.withColor(0x393D72)));
        } else if (stack.isOf(ItemInit.BABURBEN_DIMATIS)) {
            tooltip.add(Text.translatable("text.boleatte.dimatis.laide_trusting").setStyle(Style.EMPTY.withColor(0xDA9FB0)));
        } else if (stack.isOf(ItemInit.QUANTILA_DIMATIS)) {
            tooltip.add(Text.translatable("text.boleatte.dimatis.sterres_trusting").setStyle(Style.EMPTY.withColor(0xCC4818)));
        } else if (stack.isOf(ItemInit.PROTOSTERM_DIMATIS)) {
            tooltip.add(Text.translatable("text.boleatte.dimatis.oritem_trusting").setStyle(Style.EMPTY.withColor(0xD0E3ED)));
        }
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (user.isSneaking()) {
            ...
            if (user.getStackInHand(Hand.OFF_HAND).isOf(ItemInit.ARBUNE_DIMATIS)) {
                user.sendMessage(Text.translatable("text.boleatte.veret_favour.hunted_wild")
                ...
                user.sendMessage(Text.translatable("text.boleatte.beusere_virtue.prickly_rose")
                ...
                user.sendMessage(Text.translatable("text.boleatte.gela_benefit.decaying_shame")
                ...
                user.sendMessage(Text.translatable("text.boleatte.laide_blessing.gelidity_shadow")
                ...
                user.sendMessage(Text.translatable("text.boleatte.sterres_gift.heated_bitterness")
                ...
                user.sendMessage(Text.translatable("text.boleatte.oritem_profit.magical_beginnings")
        ...
        return TypedActionResult.fail(getDefaultStack());
    }
